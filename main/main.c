...
static uint8_t ext_adv_pattern_1[] = {
  0x02, 0x01, 0x06, 0x03, 0x03, 0xab, 0xcd, 0x03, 0x03, 0x18, 0x11, 0x11, 0X09,
  'b',  'l',  'e',  '-',  'v',  's',  'c',  'p',  '.',  'o',  'r',  'g',
};
#endif
...
update_advertising_data(void)
{
  ...
  if (runner % 2 == 0) {
    ...
    fields.name             = (uint8_t *) name_data;
    fields.name_len         = strlen(name_data);
    ...
    fields.mfg_data     = binary_data;
    fields.mfg_data_len = sizeof(binary_data);
  }
...
bleprph_advertise(void)
{
  ...
  // char data[20];
  // snprintf(data, sizeof(data), "Count: %d", counter++);
  // fields.name = (uint8_t *)data;
  // fields.name_len = strlen(data);
  // fields.name_is_complete = 1;
...
static void
bleprph_power_control(uint16_t conn_handle)
{
  int rc;
...
app_main(void)
{
  ...
  xTaskCreate(&eventGenerator, "main Task", 4096, NULL, 2, &numGenHandler);
}
